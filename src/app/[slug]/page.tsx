...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description =
    product.description ||
    `Handcrafted ${product.category} made from ${product.material || "premium Sri Lankan wood"}. ${product.dimensions ? `Dimensions: ${product.dimensions}.` : ""} Available for purchase with island-wide delivery.`
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const carouselImages = [product.featured_image, ...(product.gallery?.map((item) => item.image) || [])].filter(Boolean)
  ...
  const jsonLd = {
    ...
    description:
      product.description ||
      `Handcrafted ${product.category} made from ${product.material || "premium Sri Lankan wood"}.`,
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <div className="prose prose-base max-w-none text-muted-foreground leading-relaxed">
              {product.content && product.content.length > 0 ? (
                ...
                <p className="font-sans text-base leading-relaxed text-center max-w-2xl mx-auto">
                  This handcrafted wooden piece represents the finest in Sri Lankan craftsmanship. Each piece is unique
                  and made with attention to detail, showcasing traditional techniques passed down through generations.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mb-12 border-t border-border pt-8">
            ...
            {product.technicalSpecs &&
              Object.values(product.technicalSpecs).some((val) => val !== undefined && val !== null && val !== "") && (
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
                    {product.technicalSpecs.lightBulbIncluded !== undefined && (
                      <div className="text-center">
                        ...
                        <dd className="font-sans text-sm text-foreground">
                          {product.technicalSpecs.lightBulbIncluded ? "Yes" : "No"}
                        </dd>
                      </div>
                    )}
                    ...
                    {product.technicalSpecs.integratedLEDs !== undefined && (
                      <div className="text-center">
                        ...
                        <dd className="font-sans text-sm text-foreground">
                          {product.technicalSpecs.integratedLEDs ? "Yes" : "No"}
                        </dd>
                      </div>
                    )}
                    {product.technicalSpecs.dimmable !== undefined && (
                      <div className="text-center">
                        ...
                        <dd className="font-sans text-sm text-foreground">
                          {product.technicalSpecs.dimmable ? "Yes" : "No"}
                        </dd>
                      </div>
                    )}
            ...
            {product.manufacturerInfo &&
              Object.values(product.manufacturerInfo).some(
          ...
          <div className="border-t border-border pt-8">
            <div className="max-w-md mx-auto text-center">
              ...
              <div className="space-y-4 font-sans text-sm">
                <div>
                  <a href="tel:+94112345678" className="text-muted-foreground hover:text-accent transition-polene">
                    +94 11 234 5678
                  </a>
                </div>
                <div>
